...
# Plot daily closing price vs 5-day SMA on the same chart
plt.figure(figsize=(12,6))
plt.plot(data_years.index, data_years["Close"], label="Daily Closing Price", color="blue")
plt.plot(data_years.index, sma_5, label="5-Day SMA", color="red", linestyle="--")
plt.title(f"SPY Daily Closing Price vs 5-Day SMA ({years})")
plt.xlabel("Date")
plt.ylabel("Price")
plt.legend()
plt.tight_layout()
...
# Plot price chart highlighting upward and downward runs
plt.figure(figsize=(12,6))
colors = []
for i in range(1, len(close)):
	if close[i] > close[i-1]:
...
plt.plot(data_years.index, close, color='black', linewidth=1, label='Closing Price')
for i in range(1, len(close)):
	plt.plot(data_years.index[i-1:i+1], close[i-1:i+1], color=colors[i-1], linewidth=2)
plt.title(f'SPY Closing Price with Upward (green) and Downward (red) Runs ({years})')
plt.xlabel('Date')
